...
import { registerMechanic, getMechanicProfile } from "@/lib/api";
import { useAuthStore } from "@/stores/authStore";

export function MechanicRegisterPage() {
  ...
  useEffect(() => {
    getMechanicProfile(userId).then((p: { id: number } | null) => {
      if (p) navigate("/mechanic/dashboard");
    });
  }, [userId, navigate]);
    ...
    <div className="min-h-screen bg-base text-text flex flex-col">
      ...
      <main className="flex-1 p-4 sm:p-6 max-w-lg mx-auto w-full">
        <div className="flex items-center gap-2 mb-6">
          ...
          <div>
            <h1 className="text-lg font-semibold">Register as mechanic</h1>
            <p className="text-sm text-subtext">Join the Autopilot network and get jobs from customers.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <p className="text-sm font-medium text-text mb-1.5">Location (helps find nearby jobs)</p>
